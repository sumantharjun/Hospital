...
const NAV = [
  ...
  {
    label: "Results",
    href: "/operator/results",
    icon: <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>,
  },
  {
    label: "Payments",
...
export default function OperatorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== "LAB_OPERATOR") {
      router.replace("/login");
    } else {
      setUserName(user.name ?? user.email ?? "Operator");
    }
  }, [router]);
  ...
  return (
    <div className="flex min-h-screen">
      <Toaster />
      <Sidebar items={NAV} role="operator" userName={userName} />
