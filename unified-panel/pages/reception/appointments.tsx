...
export default function AppointmentsPage() {
  ...
  const doctors = bookForm.hospitalId
    ? allDoctors.filter((d: any) => (d.hospitalId || d.hospital) === bookForm.hospitalId)
    : allDoctors;
  ...
  async function bookAppointment(e: any) {
    ...
    const patient = patients.find((p: any) => (p._id || p.id) === bookForm.patientId);
    if (!patient) {
    ...
    const doctor = doctors.find((d: any) => (d._id || d.id) === bookForm.doctorId);
    ...
    const maxPerDay = (doctor as any)?.maxPatientsPerDay ?? 999;
    ...
    try {
      ...
      let rawAge = (patient as any).age != null && (patient as any).age !== "" ? Number((patient as any).age) : NaN;
      if (!Number.isFinite(rawAge) && ((patient as any).dob || (patient as any).dateOfBirth)) {
        try {
          const birth = new Date((patient as any).dob || (patient as any).dateOfBirth);
          if (!isNaN(birth.getTime())) {
            const todayDate = new Date();
            rawAge = todayDate.getFullYear() - birth.getFullYear();
            const mDiff = todayDate.getMonth() - birth.getMonth();
            if (mDiff < 0 || (mDiff === 0 && todayDate.getDate() < birth.getDate())) rawAge--;
          }
        } catch (_) {}
      }
      ...
      const payload = {
        ...
        status: "PENDING",
        patientName: (patient as any).name || "Patient",
        age: ageInRange,
        address: ((patient as any).address || "").trim() || "Not provided",
        issue: issueText,
      };
      ...
      const doc = doctors.find((d: any) => (d._id || d.id) === bookForm.doctorId);
      setPendingPayment({
        appointmentId,
        tokenNumber: tokenNum,
        ...
        patientName: (patient as any).name || "Patient",
        patientId: bookForm.patientId,
        doctorName: (doc as any)?.name || "Doctor",
        hospitalId: bookForm.hospitalId,
      });
      setPaymentForm((pf) => ({
        ...pf,
  ...
  )?.tokenNumber ?? todayApts.find((a: any) => (a.status || "").toUpperCase() === "CHECKED_IN")?.tokenNumber;

  const getPatient = (id: string) => patients.find((p: any) => (p._id || p.id) === id);
  ...
  const getDoctorName = (id: string) => allDoctors.find((d: any) => (d._id || d.id) === id)?.name || "—";
  const getDoctorNameFromApt = (apt: any) => apt?.doctorName || apt?.doctor?.name || (apt?.doctorId ? getDoctorName(apt.doctorId) : "—");
  const allDoctorsForQueue = allDoctors.length ? allDoctors : (() => {
    const ids = [...new Set(todayApts.map((a: any) => a.doctorId))];
    return ids.map((id: any) => ({ _id: id, id, name: "Doctor " + (id || "").slice(-4) }));
  })();

  const bookingDateStr = bookForm.bookingDate || today;

  function doctorAvailability(doc: any) {
              ...
              <table className="gov-table">
                ...
                <tbody>
                  {todayApts.map((a: any) => {
                    ...
                    const patientPhone = a.patient?.phone || a.patient?.phoneNumber || a.patient?.mobile || (patient as any)?.phone || (patient as any)?.phoneNumber || (patient as any)?.mobile;
                      ...
                      {step === 1 && (
                        <form onSubmit={bookAppointment} className="space-y-4 pt-2">
                          <div>
                            ...
                            <input type="date" required min={today} max={maxBookingDate} value={bookForm.bookingDate} onChange={(e) => setBookForm((f) => ({ ...f, bookingDate: e.target.value, doctorId: "", slotTime: "" }))} className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c]" />
                          ...
                          <div>
                            ...
                            <select required value={bookForm.doctorId} onChange={(e) => setBookForm((f) => ({ ...f, doctorId: e.target.value }))} className="gov-select mt-1 w-full bg-white px-4 py-2 text-[#1a202c]">
                              ...
                              {(doctors as any[]).filter((d: any) => d.isActive !== false).map((d: any) => {
                          ...
                          <div>
                            ...
                            <input type="text" required value={bookForm.issue} onChange={(e) => setBookForm((f) => ({ ...f, issue: e.target.value }))} placeholder="e.g. Fever, General check-up, Follow-up" className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c] placeholder:text-[#718096]" />
                          </div>
                          <div>
                              ...
                              <div className="mt-2 p-3 gov-card bg-[#f8fafc]">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                  {slots.map((s) => {
                                    const isBooked = bookedSlotTimes.has(s);
                                    return (
                                      <span key={s} className={`inline-flex items-center px-2 py-1 text-xs font-medium border ${isBooked ? "bg-red-50 text-red-800 border-red-600" : "bg-[#e8f5e9] text-[#2e7d32] border-[#2e7d32]"}`}>
                                        {s}
                                      </span>
                                    );
                                  })}
