...
import { BedIcon, RefreshIcon, IPAdmissionIcon } from "@/components/Icons";
...
export default function BedAvailabilityPage() {
  ...
  function getRoomInfo(bed: any) {
    const roomId = bed.roomId?._id || bed.roomId || bed.room?._id || bed.room;
    return rooms.find((r: any) => (r._id || r.id) === roomId) || bed.roomId || {};
  }
  ...
  const filteredBeds = beds.filter((bed: any) => {
    ...
    const room: any = getRoomInfo(bed);
  ...
  const grouped = filteredBeds.reduce((acc: any, bed: any) => {
    const room: any = getRoomInfo(bed);
    const key = room.roomNumber || room._id || bed.roomId || "Unassigned";
  ...
  const stats = {
    total:       beds.length,
    available:   beds.filter((b: any) => (b.status || "").toUpperCase() === "AVAILABLE").length,
    occupied:    beds.filter((b: any) => (b.status || "").toUpperCase() === "OCCUPIED").length,
    cleaning:    beds.filter((b: any) => (b.status || "").toUpperCase() === "CLEANING").length,
    maintenance: beds.filter((b: any) => (b.status || "").toUpperCase() === "MAINTENANCE").length,
  };
    ...
    return (
      <Layout user={user}>
        <div className="space-y-4">
          <h1 className="text-xl font-bold text-[#1a202c] uppercase tracking-tight">Bed Availability</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {Object.entries(fallbackStats).map(([k, v]) => (
              <div key={k} className="gov-card p-4">
                <div className="text-2xl font-bold text-[#1a202c]">{v as any}</div>
    ...
    <Layout user={user}>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-[#4a5568] mt-0.5">
              Real-time bed status across all wards — {new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
            </p>
          </div>
          <button onClick={() => fetchData(true)} disabled={refreshing}
            className="flex items-center gap-1.5 border border-[#cbd5e0] bg-white text-[#2d3748] px-3 py-2 text-sm rounded-sm hover:bg-[#e2e8f0] transition-colors disabled:opacity-50">
          ...
          <div className="flex rounded-sm overflow-hidden border border-[#cbd5e0]">
            {FILTER_TABS.map((t) => (
              <button key={t.key} onClick={() => setFilter(t.key)}
                className={`px-3 py-2 text-xs font-medium transition-colors ${filter === t.key ? "bg-[#0d47a1] text-white" : "bg-white text-[#4a5568] hover:bg-[#f5f7fa]"}`}>
                {t.label}
              </button>
            ))}
          </div>
        ...
        {Object.keys(grouped).length === 0 ? (
          ...
          <div className="space-y-6">
            {Object.entries(grouped).map(([roomKey, { room, beds: roomBeds }]: any) => (
              <div key={roomKey} className="gov-card overflow-hidden">
                <div className="px-4 py-3 bg-[#f5f7fa] border-b border-[#e2e8f0] flex items-center justify-between gap-3">
                  <div>
                    <span className="font-semibold text-[#1a202c] text-sm">Room {room.roomNumber || roomKey}</span>
                  ...
                  <div className="text-xs text-[#718096]">
                    {roomBeds.filter((b: any) => (b.status || "").toUpperCase() === "AVAILABLE").length} available / {roomBeds.length} beds
                  </div>
                </div>
                <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {roomBeds.map((bed: any) => {
                    ...
                    const patientName = bed.currentPatient?.name || bed.patient?.name || bed.patientName || null;

                    return (
                      <div key={bed._id || bed.id} className={`rounded-sm border p-3 flex flex-col gap-2 ${cfg.card}`}>
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-[#1a202c] text-sm">{bed.bedNumber || bed.number || "—"}</span>
                          <span className={`w-2.5 h-2.5 rounded-full ${cfg.dot}`} />
                        </div>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full border font-medium self-start ${cfg.bg}`}>
                          {cfg.label}
                        </span>
                        {patientName && (
                          <div className="text-[11px] text-[#4a5568] truncate" title={patientName}>{patientName}</div>
                        )}
                        {isAvailable && (
                          <button onClick={() => router.push(`/reception/ip-registration?bedId=${bed._id || bed.id}`)}
        ...
        <p className="text-xs text-[#718096] text-center">Auto-refreshes every 60 seconds.</p>
      </div>
    </Layout>
  );
}
