...
export default function DischargePage() {
  ...
  useEffect(() => {
    ...
    const ipId = router.query.ipId as string;
    if (ipId) {
      setSearchIpId(ipId);
      fetchAdmission(ipId);
    }
  ...
  const initiateDischarge = async (e: any) => {
    e.preventDefault();
    setSubmitting(true);
    try {
  ...
  const printDischargeSummary = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Discharge Summary</title>
      ...
      <div class="section">
        ...
        <div class="row"><span class="lbl">Medications:</span><span class="val">${d.medicationsOnDischarge || "—"}</span></div>
        <div class="row"><span class="lbl">Follow-up:</span><span class="val">${d.followUpInstructions || "—"}</span></div>
      </div>
      <hr>
      <div class="section">
