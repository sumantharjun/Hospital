...
const BILL_TYPE_LABELS: any = { IP: "In-Patient", OP: "Out-Patient", SERVICE: "Service" };

function printBillReceipt(bill: any) {
  const w = window.open("", "_blank");
  if (!w) return;
  const payments = (bill.paymentHistory || [])
    .map((p: any) => `<tr><td>${p.mode || "CASH"}</td><td>₹${Number(p.amount || 0).toLocaleString("en-IN")}</td><td>${p.receivedAt ? new Date(p.receivedAt).toLocaleString("en-IN") : "—"}</td></tr>`)
    .join("");
  const items = (bill.lineItems || [])
    .map((li: any) => `<tr><td>${li.description || li.type}</td><td>₹${Number(li.amount || li.unitPrice || 0).toLocaleString("en-IN")}</td></tr>`)
    .join("");
