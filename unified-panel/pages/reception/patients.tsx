...
export default function PatientsPage() {
  ...
  function setLocalPatientDetails(id: string, details: any) {
    if (!id) return;
  ...
  async function searchByMobile() {
    ...
    try {
      ...
      match ? toast.success("Returning patient found.") : toast("No patient with this mobile/Aadhaar. You can add new.");
    } catch (e) {
  ...
  async function addPatient(e: any) {
    ...
    const existingByMobile = patients.find((p: any) => (p.phone || p.mobile || "").replace(/\D/g, "").slice(-10) === mobileNorm);
    if (existingByMobile) {
    ...
    if (form.aadhaar && form.aadhaar.replace(/\D/g, "").length >= 4) {
      ...
      if (patients.find((p: any) => (p.aadhaar || "").replace(/\D/g, "").slice(-4) === aadhaarNorm)) {
    ...
    try {
      const hospital = hospitals.find((h: any) => (h._id || h.id) === form.hospitalId);
      const hospitalCode = (hospital as any)?.branchCode || (hospital as any)?.name?.slice(0, 6).toUpperCase().replace(/\s/g, "") || "OPD";
      const opdNumber = formatOpdNumber(hospitalCode, "00", patients.length + 1);
  ...
  const filteredList = searchMobile.trim()
    ? patients.filter((p: any) => (p.phone || p.phoneNumber || p.mobile || "").includes(searchMobile) || (p.name || "").toLowerCase().includes(searchMobile.toLowerCase()) || (p.email || "").toLowerCase().includes(searchMobile.toLowerCase()) || (p.aadhaar || "").includes(searchMobile))
    : patients;
      ...
      <div className="space-y-6">
        ...
        <div className="gov-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="gov-table">
              ...
              <tbody>
                {loading ? <tr><td colSpan={9} className="text-center py-8"><div className="flex justify-center"><LoadingSpinner /></div><p className="mt-2 text-sm text-[#718096]">Loading patients…</p></td></tr> : filteredList.length === 0 ? <tr><td colSpan={9} className="text-center text-[#718096] py-8">No patients found.</td></tr> : filteredList.map((p: any) => {
                  const id = p._id || p.id;
                  const merged = { ...p, ...localDetails[id] };
                  const np = normalizePatient(merged);
                  return (
              ...
              <form onSubmit={addPatient} className="mt-6 space-y-4">
                <div>
                  ...
                  <select value={form.hospitalId} onChange={(e) => setForm((f) => ({ ...f, hospitalId: e.target.value }))} className="gov-select mt-1 w-full bg-white px-4 py-2 text-[#1a202c]">
                    ...
                    {hospitals.map((h: any) => <option key={h._id || h.id} value={h._id || h.id}>{h.name} {h.branchCode ? `(${h.branchCode})` : ""}</option>)}
                  </select>
                </div>
                <div>
        ...
        {editPatient && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => !editSaving && setEditPatient(null)}>
            <div className="w-full max-w-md gov-card p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <h2 className="text-base font-bold text-[#1a202c] border-b border-[#cbd5e0] pb-2">Edit Patient — {editPatient.name}</h2>
              <p className="text-xs text-[#718096] mt-1 mb-4">OPD: {editPatient.opdNumber || editPatient._id}</p>
              <form onSubmit={saveEdit} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-[#2d3748]">Full name</label>
                  <input value={editForm.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c]" placeholder="Full name" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-[#2d3748]">Mobile</label>
                  <input type="tel" value={editForm.phone} onChange={(e) => setEditForm((f) => ({ ...f, phone: e.target.value }))} className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c]" placeholder="9876543210" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-[#2d3748]">Age</label>
                    <input type="number" min="1" max="120" value={editForm.age} onChange={(e) => setEditForm((f) => ({ ...f, age: e.target.value }))} className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c]" placeholder="25" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-[#2d3748]">Gender</label>
                    <select value={editForm.gender} onChange={(e) => setEditForm((f) => ({ ...f, gender: e.target.value }))} className="gov-select mt-1 w-full bg-white px-4 py-2 text-[#1a202c]">
                      <option value="">Select</option>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                      <option value="Other">Other</option>
                ...
                <div>
                  ...
                  <select value={editForm.bloodGroup} onChange={(e) => setEditForm((f) => ({ ...f, bloodGroup: e.target.value }))} className="gov-select mt-1 w-full bg-white px-4 py-2 text-[#1a202c]">
                    <option value="">—</option>
                    {["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"].map((b) => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-[#2d3748]">Address</label>
                  <textarea value={editForm.address} onChange={(e) => setEditForm((f) => ({ ...f, address: e.target.value }))} rows={2} className="gov-input mt-1 w-full bg-white px-4 py-2 text-[#1a202c]" placeholder="Address" />
                </div>
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setEditPatient(null)} disabled={editSaving} className="flex-1 border border-[#cbd5e0] py-2.5 text-sm font-medium text-[#2d3748] bg-white hover:bg-[#f8fafc]">Cancel</button>
                  <button type="submit" disabled={editSaving} className="flex-1 gov-btn-primary py-2.5 disabled:opacity-50">{editSaving ? "Saving…" : "Save changes"}</button>
                </div>
              </form>
            </div>
          </div>
