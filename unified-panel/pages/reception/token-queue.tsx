...
import { RefreshIcon, CheckIcon, AlertIcon, PrintIcon } from "@/components/Icons";
...
export default function TokenQueuePage() {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const todayApts = Array.isArray(apts) ? apts.filter((a: any) => {
        const d = a.appointmentDate || a.scheduledAt;
        return d && new Date(d).toISOString().split("T")[0] === today;
      }) : [];
      setAppointments(todayApts.sort((a: any, b: any) => (a.tokenNumber || 0) - (b.tokenNumber || 0)));
  ...
  async function updateStatus(apt: any) {
  ...
  async function cancelAppointment(apt: any) {
  ...
  function waitingMinutes(apt: any) {
  ...
  const stats = {
    ...
    waiting:      appointments.filter((a: any) => ["PENDING","CHECKED_IN"].includes((a.status||"").toUpperCase())).length,
    withDoctor:   appointments.filter((a: any) => (a.status||"").toUpperCase() === "IN_PROGRESS").length,
    done:         appointments.filter((a: any) => (a.status||"").toUpperCase() === "COMPLETED").length,
    longWait:     appointments.filter((a: any) => ["PENDING","CHECKED_IN"].includes((a.status||"").toUpperCase()) && waitingMinutes(a) > 30).length,
  };

  const currentToken = appointments.find((a: any) => (a.status||"").toUpperCase() === "IN_PROGRESS");
  const nextToken    = appointments.find((a: any) => (a.status||"").toUpperCase() === "CHECKED_IN");
      ...
      <div className="space-y-6">
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-[#0d47a1] rounded-sm flex items-center justify-center text-white font-bold text-xl">
                  {(currentToken as any).tokenNumber ?? "—"}
                </div>
                <div>
                  <div className="text-xs font-semibold text-[#0d47a1] uppercase tracking-wide">Now Serving</div>
                  <div className="font-bold text-[#1a202c]">{(currentToken as any).patientName || (currentToken as any).patient?.name || "Patient"}</div>
                  <div className="text-sm text-[#4a5568]">{(currentToken as any).doctorName || "Doctor"}</div>
                </div>
              </div>
              {nextToken && (
                <div className="text-right">
                  <div className="text-xs text-[#718096]">Next</div>
                  <div className="text-2xl font-bold text-[#2d3748]">#{(nextToken as any).tokenNumber}</div>
                  <div className="text-xs text-[#4a5568]">{(nextToken as any).patientName || (nextToken as any).patient?.name || ""}</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          <select value={selectedDoctor} onChange={(e) => setSelectedDoctor(e.target.value)}
            ...
            {doctors.map((d: any) => <option key={d._id || d.id} value={d._id || d.id}>Dr. {d.name}</option>)}
          </select>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#e2e8f0]">
                {displayApts.map((apt: any) => {
                  ...
                  const rowBg = isLongWait ? "bg-red-50" : isMedWait ? "bg-yellow-50/40" : status === "IN_PROGRESS" ? "bg-blue-50/50" : "";
                  return (
                    <tr key={apt._id || apt.id} className={`transition-colors hover:brightness-95 ${rowBg}`}>
