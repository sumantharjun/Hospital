...
import { RefreshIcon, StethoscopeIcon, AppointmentsIcon, CheckIcon, AlertIcon } from "@/components/Icons";
import Layout from "@/components/Layout";

const AVAILABILITY_CONFIG: any = {
...
function deriveStatus(stats: any, manualStatus: string | null) {
...
export default function DoctorDutyBoardPage() {
  ...
  function getDoctorStats(doctor: any) {
    const id = doctor._id || doctor.id;
    const docApts = appointments.filter((a: any) => {
      const dId = a.doctorId || a.doctor?._id || a.doctor;
      return dId === id;
    });
    const total = docApts.length;
    const waiting = docApts.filter((a: any) => ["PENDING", "CHECKED_IN"].includes((a.status || "").toUpperCase())).length;
    const inProgress = docApts.filter((a: any) => (a.status || "").toUpperCase() === "IN_PROGRESS").length;
    const completed = docApts.filter((a: any) => (a.status || "").toUpperCase() === "COMPLETED").length;
    const waitTime = waiting * 10;
    return { total, waiting, inProgress, completed, waitTime };
  }

  function toggleManualStatus(id: string, current: string | null) {
    setManualStatus((prev: any) => {
      const next = { ...prev };
  ...
  const sortOrder: any = { BUSY: 0, AVAILABLE: 1, BREAK: 2, UNAVAILABLE: 3 };
  const sorted = [...doctorsWithStats].sort((a: any, b: any) => (sortOrder[a.status] ?? 4) - (sortOrder[b.status] ?? 4));
    ...
    return (
      <Layout user={user}>
        <div className="space-y-4">
          <h1 className="text-xl font-bold text-[#1a202c] uppercase tracking-tight">Doctor Duty Board</h1>
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-[#4a5568] mt-0.5">
              Live doctor availability &amp; queue depth — {new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "short" })}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: "Available",     value: sorted.filter((d: any) => d.status === "AVAILABLE").length,                               bg: "bg-green-50 border border-green-200",  text: "text-green-800" },
            { label: "Busy",          value: sorted.filter((d: any) => d.status === "BUSY").length,                                    bg: "bg-orange-50 border border-orange-200",text: "text-orange-800"},
            { label: "Away / Break",  value: sorted.filter((d: any) => ["BREAK","UNAVAILABLE"].includes(d.status)).length,             bg: "bg-gray-50 border border-gray-200",    text: "text-gray-600"  },
          ].map((s) => (
        ...
        {sorted.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sorted.map((doc: any) => {
              ...
              const manualCurrent = doc.status === "BREAK" || doc.status === "UNAVAILABLE" ? doc.status : null;

              return (
                <div key={id} className={`gov-card p-5 flex flex-col gap-4 border-t-4 ${
                  doc.status === "BUSY" ? "border-orange-400" :
                  doc.status === "AVAILABLE" ? "border-green-400" : "border-gray-300"
                }`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-[#1a202c]">Dr. {doc.name || "Unknown"}</div>
                        <div className="text-xs text-[#718096]">{doc.specialization || doc.department || "General"}</div>
                      </div>
                    </div>
                    <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border font-semibold shrink-0 ${cfg.bg}`}>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    {doc.stats.total === 0 ? (
                      ...
                      <>
                        ...
                        <span className={`text-xs font-semibold ${doc.stats.waitTime > 30 ? "text-red-600" : doc.stats.waitTime > 15 ? "text-orange-600" : "text-green-700"}`}>
                  ...
                  <div className="flex gap-2 pt-1 border-t border-[#e2e8f0]">
                    <button onClick={() => router.push(`/reception/appointments?doctorId=${id}`)}
                    ...
                    <button onClick={() => toggleManualStatus(id, manualCurrent)}
                      title={manualCurrent === "BREAK" ? "Mark Unavailable" : manualCurrent === "UNAVAILABLE" ? "Mark Available" : "Mark on Break"}
        ...
        <p className="text-xs text-[#718096] text-center">Auto-refreshes every 30 seconds. Availability toggle is local UI only.</p>
      </div>
    </Layout>
  );
}
