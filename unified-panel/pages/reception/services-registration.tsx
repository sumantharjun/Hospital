...
export default function ServicesRegistrationPage() {
  ...
  const [createdReg, setCreatedReg] = useState<any>(null);
  ...
  const fetchServicesForHospital = async (hid: string) => {
    const headers = getAuthHeaders();
  ...
  const searchPatients = (q: string) => {
    ...
    setPatientResults((patients as any[]).filter((p: any) => p.name?.toLowerCase().includes(lower) || p.phone?.includes(q)).slice(0, 6));
  };
  ...
  const removeService = (serviceId: string) => setSelectedServices(p => p.filter(s => s.serviceId !== serviceId));

  const subtotal = selectedServices.reduce((s, i) => s + i.fee, 0);
  const taxTotal = selectedServices.reduce((s, i) => s + i.taxAmount, 0);
  const grandTotal = subtotal + taxTotal;

  const filteredServices = (services as any[]).filter((s: any) => !serviceCategory || s.category === serviceCategory);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1"
              ...
              {(hospitals as any[]).map((h: any) => <option key={h._id} value={h._id}>{h.name}</option>)}
            </select>
          </div>
