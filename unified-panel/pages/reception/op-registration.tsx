...
export default function OPRegistrationPage() {
  ...
  const [createdOP, setCreatedOP] = useState<any>(null);
  ...
  const selectDoctor = (doctorId: string) => {
    const doc: any = (doctors as any[]).find((d: any) => d._id === doctorId);
    setForm(p => ({
      ...p,
      doctorId,
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
            ...
            <div>
              ...
              <select required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1"
                ...
                {(doctors as any[]).map((d: any) => <option key={d._id} value={d._id}>Dr. {d.name} — {d.specialization} (₹{d.serviceCharge || 0})</option>)}
              </select>
            </div>
