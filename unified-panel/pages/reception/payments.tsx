...
export default function PaymentsPage() {
  ...
  const [summary, setSummary] = useState({ totalBilled: 0, totalCollected: 0, totalPending: 0, byMode: {} as any });
    ...
    try {
      ...
      if (billsRes.ok) {
        ...
        arr.forEach((bill: any) => {
          (bill.paymentHistory || []).forEach((pay: any) => {
  ...
  useEffect(() => { if (user) fetchPayments(); }, [fetchPayments, user]);

  const grandTotal = transactions.reduce((s, p) => s + Number(p.amount || 0), 0);
  const byMode = transactions.reduce((acc: any, p: any) => {
