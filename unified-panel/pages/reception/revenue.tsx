...
export default function RevenuePage() {
  ...
  useEffect(() => { if (user) fetchRevenue(); }, [fetchRevenue, user]);
  ...
  function downloadCSV() {
    ...
    const lines = [
      ...
      ...(revenue.byType ? Object.entries(revenue.byType).map(([k, v]: any) => `${k},${v.billed || 0},${v.collected || 0}`) : []),
      "",
      "Payment Mode,Amount",
      ...(revenue.byMode ? Object.entries(revenue.byMode).map(([k, v]) => `${k},${v}`) : []),
      "",
      "Doctor,OP Count,IP Count,OP Revenue,IP Revenue,Total Revenue",
      ...(doctorRevenue as any[]).map((d: any) => `${d.doctorName || d.doctorId},${d.opCount},${d.ipCount},${d.opRevenue},${d.ipRevenue},${d.totalRevenue}`),
    ];
              ...
              <div className="bg-white border border-[#e2e8f0] rounded-sm shadow-sm overflow-hidden">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#e2e8f0]">
                    ...
                    ) : Object.entries(revenue.byType || {}).map(([type, v]: any) => (
