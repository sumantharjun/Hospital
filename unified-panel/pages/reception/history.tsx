...
export default function HistoryPage() {
  ...
  const getDoctorName = (id: string) => (doctors as any[]).find((d: any) => (d._id || d.id) === id)?.name || "—";

  const patientVisits = selectedPatient
    ? (appointments as any[])
        .filter((a: any) => a.patientId === (selectedPatient._id || selectedPatient.id))
        .sort((a: any, b: any) => new Date(b.scheduledAt || b.appointmentDate).getTime() - new Date(a.scheduledAt || a.appointmentDate).getTime())
    : [];
  ...
  function doSearch() {
    ...
    const byPhone = (patients as any[]).find((p: any) => (p.phone || p.mobile || "").toString().replace(/\D/g, "").includes(q.replace(/\D/g, "")));
    const byName = (patients as any[]).find((p: any) => (p.name || "").toLowerCase().includes(q));
    const byId = (patients as any[]).find((p: any) => (p._id || p.id || "").toString().toLowerCase().includes(q));
  ...
  return (
    <Layout user={user}>
      <div className="space-y-6">
        ...
        {loading && <LoadingOverlay text="Loading history…" />}
              ...
              <div className="overflow-x-auto">
                {patientVisits.length === 0 ? (
                  ...
                  <table className="gov-table">
                    ...
                    <tbody>
                      {patientVisits.slice(0, 50).map((a: any) => {
