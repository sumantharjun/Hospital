...
function StatCard({ label, value, sub, icon, accent, onClick }: any) {
  return (
    <div
      onClick={onClick}
...
export default function DashboardPage() {
  ...
  const [recentIP, setRecentIP] = useState<any[]>([]);
  const [recentOP, setRecentOP] = useState<any[]>([]);
  ...
  const fetchAll = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const apts = (aptsRes as any)?.ok ? await (aptsRes as any).json() : [];
      const doctors = (doctorsRes as any)?.ok ? await (doctorsRes as any).json() : [];
      const hosps = (hospRes as any)?.ok ? await (hospRes as any).json() : [];
      const ipData = (ipRes as any)?.ok ? await (ipRes as any).json() : null;
      const opData = (opRes as any)?.ok ? await (opRes as any).json() : null;
      const svcRaw = (svcRes as any)?.ok ? await (svcRes as any).json() : [];
      const bedData = (bedRes as any)?.ok ? await (bedRes as any).json() : null;
      const revenue = (revenueRes as any)?.ok ? await (revenueRes as any).json() : null;
      const billsRaw = (billsRes as any)?.ok ? await (billsRes as any).json() : [];
      const patientsRaw = (patientsRes as any)?.ok ? await (patientsRes as any).json() : [];
      ...
      const ipList = (ipListRes as any)?.ok ? await (ipListRes as any).json() : [];
      ...
      setStats({
        ...
        waiting: todayApts.filter((a: any) => ["PENDING", "CHECKED_IN"].includes((a.status || "").toUpperCase())).length,
        withDoctor: todayApts.filter((a: any) => (a.status || "").toUpperCase() === "IN_PROGRESS").length,
        completed: todayApts.filter((a: any) => (a.status || "").toUpperCase() === "COMPLETED").length,
  ...
  const grandCollected = bills.reduce((s: number, b: any) => s + (b.paidAmount || 0), 0);
  const byMode = bills.reduce((acc: any, b: any) => {
