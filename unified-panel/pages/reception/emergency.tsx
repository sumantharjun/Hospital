...
import { AlertIcon, RefreshIcon, CheckIcon, PrintIcon, BedIcon, StethoscopeIcon } from "@/components/Icons";
        ...
        <div key={s} className="flex items-center">
          <div className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 ${
              i < current ? "bg-[#0d47a1] border-[#0d47a1] text-white" :
              i === current ? "bg-white border-[#0d47a1] text-[#0d47a1]" :
              "bg-white border-[#cbd5e0] text-[#718096]"
            }`}>
              {i < current ? <CheckIcon className="w-3.5 h-3.5" /> : i + 1}
            </div>
            <span className={`text-xs font-medium hidden sm:inline ${i === current ? "text-[#0d47a1]" : "text-[#718096]"}`}>{s}</span>
          </div>
          {i < STEPS.length - 1 && (
...
export default function EmergencyPage() {
  ...
  const fetchSupportData = useCallback(async () => {
    ...
    try {
      ...
      if (bedRes.ok) {
        ...
        setAvailableBeds(bedList.filter((bed: any) => (bed.status || "").toUpperCase() === "AVAILABLE"));
      }
      if (emRes.ok) {
        const em = await emRes.json();
        const list = Array.isArray(em) ? em : em.admissions || [];
        const sorted = [...list].sort((a: any, b: any) => new Date(b.admissionDate || b.createdAt).getTime() - new Date(a.admissionDate || a.createdAt).getTime());
  ...
  function resetForm() {
    setStep(0); setTriageLevel(""); setChiefComplaint(""); setPatientName(""); setAge(""); setGender(""); setPhone(""); setConditions(""); setAllergies(""); setDoctorId(""); setBedId(""); setNotes(""); setSuccess(null);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(buildApiUrl("/api/ip-admissions"), {
        ...
        body: JSON.stringify({
          emergencyFlag: true, casualtyFlag: true,
          patientName: patientName.trim(), age: parseInt(age, 10), gender,
          phone: phone.trim() || undefined, chiefComplaint: chiefComplaint.trim(), triageLevel,
          doctorId: doctorId || undefined, bedId: bedId || undefined,
          notes: notes.trim() || undefined, admissionDate: new Date().toISOString(),
          knownConditions: conditions.trim() || undefined, allergies: allergies.trim() || undefined,
        }),
      });
      if (!res.ok) {
      ...
      const ipId = data.ipId || data.admissionId || data.admissionNumber || data._id || data.id || "GENERATED";
  ...
  function printConfirmation(ipId: string, name: string) {
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-sm text-[#4a5568] mt-0.5">Fast triage registration for emergency patients</p>
            </div>
            {emergencyCount > 0 && (
        ...
        {success ? (
          <div className="gov-card p-8 text-center border-t-4 border-green-500">
            ...
            <p className="text-sm text-[#4a5568] mb-4">Emergency admission created for <strong>{success.patientName}</strong></p>
            <div className="inline-block bg-red-50 border-2 border-red-300 rounded-sm px-8 py-4 mb-6">
              <div className="text-xs font-semibold text-red-700 uppercase tracking-wide mb-1">IP Admission ID</div>
              <div className="text-3xl font-black text-red-700">{success.ipId}</div>
            </div>
            <div className="flex justify-center gap-3 flex-wrap">
              <button onClick={() => printConfirmation(success.ipId, success.patientName)}
            ...
            <div className="p-5 space-y-5">
              {step === 0 && (
                <div className="space-y-5">
                  <div>
                    <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-3">Select Triage Level *</label>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {TRIAGE_LEVELS.map((t) => (
                        <button key={t.key} type="button" onClick={() => setTriageLevel(t.key)}
                          className={`rounded-sm border-2 p-4 text-center transition-all ${triageLevel === t.key ? `${t.bg} ${t.border} ${t.text} shadow-md scale-[1.02]` : `bg-white border-[#e2e8f0] text-[#4a5568] hover:border-gray-400`}`}>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Chief Complaint *</label>
                  ...
                  <div className="flex justify-end">
                    <button onClick={() => { if (!triageLevel) { toast.error("Please select a triage level"); return; } setStep(1); }}
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Patient Name *</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Age *</label>
                      <input type="number" value={age} onChange={(e) => setAge(e.target.value)} placeholder="Age in years" min="0" max="150"
                        className="w-full border border-[#cbd5e0] rounded-sm px-3 py-2 text-sm text-[#1a202c] focus:outline-none focus:ring-2 focus:ring-[#0d47a1]/30 focus:border-[#0d47a1]" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Gender *</label>
                      <select value={gender} onChange={(e) => setGender(e.target.value)} className="gov-select w-full">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Phone</label>
                      <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Contact number"
                        className="w-full border border-[#cbd5e0] rounded-sm px-3 py-2 text-sm text-[#1a202c] focus:outline-none focus:ring-2 focus:ring-[#0d47a1]/30 focus:border-[#0d47a1]" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Known Conditions</label>
                      <input type="text" value={conditions} onChange={(e) => setConditions(e.target.value)} placeholder="Diabetes, hypertension..."
                        className="w-full border border-[#cbd5e0] rounded-sm px-3 py-2 text-sm text-[#1a202c] focus:outline-none focus:ring-2 focus:ring-[#0d47a1]/30 focus:border-[#0d47a1]" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Allergies</label>
                      <input type="text" value={allergies} onChange={(e) => setAllergies(e.target.value)} placeholder="Penicillin, sulfa drugs..."
                        className="w-full border border-[#cbd5e0] rounded-sm px-3 py-2 text-sm text-[#1a202c] focus:outline-none focus:ring-2 focus:ring-[#0d47a1]/30 focus:border-[#0d47a1]" />
                    </div>
                  </div>
                  <div className="flex justify-between gap-3">
                    <button onClick={() => setStep(0)} className="border border-[#cbd5e0] text-[#4a5568] px-4 py-2.5 rounded-sm text-sm hover:bg-[#f5f7fa] transition-colors">Back</button>
                    <button onClick={() => {
                      if (!patientName.trim()) { toast.error("Patient name is required"); return; }
                      if (!age) { toast.error("Age is required"); return; }
                      if (!gender) { toast.error("Gender is required"); return; }
              ...
              {step === 2 && (
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Assign Doctor</label>
                      <select value={doctorId} onChange={(e) => setDoctorId(e.target.value)} className="gov-select w-full">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Assign Bed</label>
                      <select value={bedId} onChange={(e) => setBedId(e.target.value)} className="gov-select w-full">
                        <option value="">Select bed (optional)</option>
                        {availableBeds.map((b: any) => (
                          <option key={b._id || b.id} value={b._id || b.id}>
                            Bed {b.bedNumber || b.number}{b.roomId?.roomNumber ? ` — Room ${b.roomId.roomNumber}` : ""}
                          </option>
                        ))}
                      </select>
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-semibold text-[#4a5568] uppercase tracking-wide mb-1.5">Additional Notes</label>
                  ...
                  <div className="flex justify-between gap-3">
                    <button onClick={() => setStep(1)} className="border border-[#cbd5e0] text-[#4a5568] px-4 py-2.5 rounded-sm text-sm hover:bg-[#f5f7fa] transition-colors">Back</button>
        ...
        <div className="gov-card overflow-hidden">
          <div className="px-5 py-3 border-b border-[#e2e8f0] flex items-center justify-between bg-red-50">
            ...
            {emergencyCount > 0 && (
              <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded-full font-bold">{emergencyCount} active</span>
            )}
          </div>
                    ...
                    return (
                      <tr key={em._id || em.id} className="hover:bg-[#f5f7fa]">
                        ...
                        <td className="px-4 py-3">
                          {tCfg ? (
                            <span className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full border font-semibold ${tCfg.badge}`}>{tCfg.label}</span>
                          ) : (
                            <span className="text-xs text-[#718096]">{em.triageLevel || "—"}</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-[#4a5568] text-xs hidden sm:table-cell max-w-[180px] truncate">{em.chiefComplaint || "—"}</td>
                        <td className="px-4 py-3 text-xs text-[#718096] hidden md:table-cell">
                          {em.admissionDate || em.createdAt
                            ? new Date(em.admissionDate || em.createdAt).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
                            : "—"}
                        </td>
