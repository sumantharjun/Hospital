...
export default function IPRegistrationPage() {
  ...
  const [createdIP, setCreatedIP] = useState<any>(null);
  ...
  const fetchForHospital = async (hid: string) => {
    const headers = getAuthHeaders();
  ...
  const fetchBedsForRoom = async (roomId: string) => {
    const room: any = availableRooms.find((r: any) => r._id === roomId);
  ...
  const searchPatients = (q: string) => {
    ...
    setPatientResults((patients as any[]).filter((p: any) => p.name?.toLowerCase().includes(lower) || p.phone?.includes(q)).slice(0, 6) as any);
  };
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (!form.patientId && !form.newPatient) { toast.error("Please select or create a patient"); return; }
    ...
    try {
      ...
      if (form.newPatient) {
        const pRes = await fetch(buildApiUrl("/api/users/signup"), {
          ...
          body: JSON.stringify({
            name: form.patientName, email: `patient_${Date.now()}@hsp.local`, password: "Patient@123",
            role: "PATIENT", phone: form.patientContact, age: form.newPatientAge,
            gender: form.newPatientGender, address: form.newPatientAddress,
            bloodGroup: form.newPatientBloodGroup, hospitalId: form.hospitalId,
          }),
        });
      ...
      const res = await fetch(buildApiUrl("/api/ip-admissions"), {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify({ ...form, patientId, admissionDate: new Date(form.admissionDate) }),
      });
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1" value={form.hospitalId}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
              ...
              {!form.newPatient ? (
                <div className="relative">
                  ...
                  <input type="text" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1" placeholder="Search by name or phone..."
                    value={searchPatient} onChange={(e) => searchPatients(e.target.value)} />
                  {patientResults.length > 0 && (
                    <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1">
                      {patientResults.map((p: any) => (
                        <div key={p._id} onClick={() => selectPatient(p)} className="px-4 py-3 hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-0">
                  ...
                  {form.patientId && <div className="mt-2 p-2 bg-blue-50 rounded text-xs text-blue-700">✓ Patient selected: {form.patientName}</div>}
                </div>
              ) : (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
              ...
              <div>
                ...
                <select required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1"
                  ...
                  {doctors.map((d: any) => <option key={d._id} value={d._id}>Dr. {d.name} — {d.specialization}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                <select required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mt-1"
                  ...
                  {availableRooms.map((r: any) => (
                    <option key={r._id} value={r._id}>Room {r.roomNumber} (Floor {r.floor}) — {r.roomType} — ₹{r.rentPerDay}/day</option>
                  ))}
                </select>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  ...
                  ["Doctor", (doctors as any[]).find((d: any) => d._id === form.doctorId)?.name || "—"],
                  ["Room", (availableRooms as any[]).find((r: any) => r._id === form.roomId)?.roomNumber || "—"],
                  ["Bed", (availableBeds as any[]).find((b: any) => b._id === form.bedId)?.bedNumber || "—"],
                  ...
                  <div key={String(k)} className="flex justify-between border-b border-gray-50 py-1">
