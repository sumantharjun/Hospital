...
export default function InvoicesPage() {
  ...
  function printOpdReceipt(receipt: any) {
    if (!receipt) return;
  ...
  function deleteReceipt(receipt: any) {
    ...
    if (typeof window !== "undefined") {
      try { localStorage.setItem(RECEIPTS_KEY, JSON.stringify(next)); } catch {}
    }
    setReceipts(next);
  ...
  function updateManualItem(index: number, field: string, value: string) {
    setManualInvoice((m) => ({
      ...m,
      items: m.items.map((row: any, i: number) => (i === index ? { ...row, [field]: value } : row)),
    }));
  }

  function removeManualItem(index: number) {
    setManualInvoice((m) => ({ ...m, items: m.items.filter((_: any, i: number) => i !== index) }));
  }
  ...
  function updateMedicineRow(index: number, field: string, value: string) {
    setManualInvoice((m) => ({
      ...m,
      medicines: m.medicines.map((row: any, i: number) => (i === index ? { ...row, [field]: value } : row)),
    }));
  }

  function removeMedicineRow(index: number) {
    setManualInvoice((m) => ({ ...m, medicines: m.medicines.filter((_: any, i: number) => i !== index) }));
  }
  ...
  function generateManualInvoice() {
    ...
    const items = m.items.filter((row: any) => (row.description || "").trim() || Number(row.amount) > 0);
    const medicines = (m.medicines || []).filter((row: any) => (row.name || "").trim() || Number(row.quantity) > 0 || Number(row.amount) > 0);
    if (items.length === 0 && medicines.length === 0) {
      toast.error("Add at least one line item or medicine with amount");
      return;
    }
    const itemsTotal = items.reduce((sum: number, row: any) => sum + (Number(row.amount) || 0), 0);
    const medicinesTotal = medicines.reduce((sum: number, row: any) => sum + (Number(row.amount) || 0), 0);
    ...
    <Layout user={user}>
      <div className="space-y-6">
        ...
        <div className="flex gap-2 border-b-2 border-[#cbd5e0]">
          {[
            { key: "receipts", label: "OPD receipts" },
            { key: "order", label: "Download order PDF" },
            { key: "manual", label: "Generate invoice" },
          ].map((tab) => (
            <button
              key={tab.key}
              type="button"
              onClick={() => setActiveSection(tab.key)}
              className={`px-4 py-2 text-sm font-semibold border-b-2 -mb-[2px] transition-colors ${activeSection === tab.key ? "border-[#0d47a1] text-[#0d47a1]" : "border-transparent text-[#718096] hover:text-[#1a202c]"}`}
            >
              {tab.label}
            </button>
          ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={manualInvoice.invoiceNumber} onChange={(e) => setManualInvoice((m) => ({ ...m, invoiceNumber: e.target.value }))} placeholder="INV-20240224-001" className="gov-input w-full bg-white px-4 py-2 text-[#1a202c]" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={manualInvoice.customerName} onChange={(e) => setManualInvoice((m) => ({ ...m, customerName: e.target.value }))} placeholder="Full name" className="gov-input w-full bg-white px-4 py-2 text-[#1a202c]" />
                </div>
                <div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addManualItem} className="text-sm text-[#0d47a1] hover:underline font-medium">+ Add row</button>
                </div>
                <div className="border border-[#cbd5e0] rounded overflow-hidden">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-[#e2e8f0]">
                      {manualInvoice.items.map((row: any, i: number) => (
                        <tr key={i}>
                          <td className="px-3 py-2 text-[#4a5568]">{i + 1}</td>
                          <td className="px-3 py-2">
                            <input type="text" value={row.description} onChange={(e) => updateManualItem(i, "description", e.target.value)} placeholder="e.g. Consultation" className="w-full border border-[#cbd5e0] bg-white px-2 py-1.5 text-[#1a202c] text-sm rounded" />
                          </td>
                          <td className="px-3 py-2">
                            <input type="number" min="0" step="0.01" value={row.amount} onChange={(e) => updateManualItem(i, "amount", e.target.value)} placeholder="0" className="w-full border border-[#cbd5e0] bg-white px-2 py-1.5 text-[#1a202c] text-sm text-right rounded" />
                          </td>
                          <td className="px-2 py-2">
                            <button type="button" onClick={() => removeManualItem(i)} className="text-red-600 hover:text-red-700 text-xs p-1" title="Remove row">✕</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <p className="mt-2 text-sm text-[#4a5568]">
                  Charges total: <strong className="text-[#1a202c]">₹{manualInvoice.items.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0).toLocaleString("en-IN")}</strong>
                </p>
                ...
                <div className="mt-6 flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={addMedicineRow} className="text-sm text-[#0d47a1] hover:underline font-medium">+ Add medicine</button>
                </div>
                <div className="border border-[#cbd5e0] rounded overflow-hidden">
                  <table className="min-w-full text-sm">
                    <thead className="bg-[#f8fafc]">
                    ...
                    <tbody className="divide-y divide-[#e2e8f0]">
                      {(manualInvoice.medicines || []).map((row: any, i: number) => (
                        <tr key={i}>
                          <td className="px-3 py-2 text-[#4a5568]">{i + 1}</td>
                          <td className="px-3 py-2">
                ...
                <p className="mt-2 text-sm text-[#4a5568]">
                  Grand total: <strong className="text-[#1a202c]">₹{(manualInvoice.items.reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0) + (manualInvoice.medicines || []).reduce((s: number, r: any) => s + (Number(r.amount) || 0), 0)).toLocaleString("en-IN")}</strong>
                </p>
