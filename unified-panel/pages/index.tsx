...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        const res = await fetch(buildApiUrl(`/api/users/check-role/${encodeURIComponent(email)}`));
        if (res.ok) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-teal-50 relative overflow-hidden">
        ...
        <div className="max-w-sm sm:max-w-none ml-auto">
          <Toaster
            ...
            toastOptions={{
              ...
              error:   { iconTheme: { primary: "#DC3545", secondary: "#fff" }, style: { borderLeft: "4px solid #DC3545" } },
            }}
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <div className="h-16 w-16 sm:h-20 sm:w-20 rounded-3xl bg-gradient-to-br from-[#0ea5a4] to-[#0f766e] flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shadow-2xl shadow-teal-500/50 mx-auto">
              HP
            </div>
          </motion.div>
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-6">
            Sign in to access your portal — Admin, Reception, or Lab.
          </p>
          <form onSubmit={handleSubmit} className="space-y-5">
            <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
              ...
              <input
                type="email"
                className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-black outline-none focus:ring-2 focus:ring-[#0ea5a4] focus:border-[#0ea5a4] transition-all"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="Enter your email address"
                required
              />
              {checkingRole && <p className="text-xs text-gray-600 mt-1">Checking access...</p>}
              {isAdminLogin && !checkingRole && (
                <motion.div
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-2 px-3 py-2 bg-teal-50 border border-teal-200 rounded-lg"
                >
            ...
            <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }}>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 px-4 py-2.5 pr-10 text-sm text-black outline-none focus:ring-2 focus:ring-[#0ea5a4] focus:border-[#0ea5a4] transition-all"
                  value={password}
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full rounded-lg px-4 py-2.5 font-semibold text-sm text-white disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer transition-all bg-gradient-to-r from-[#0ea5a4] to-[#0f766e] hover:from-[#0f766e] hover:to-[#0b6b6b] shadow-lg"
            >
              {loading ? (
          ...
          >
            ...
            <button
              ...
              className="text-[#0ea5a4] hover:text-[#0f766e] underline font-medium transition-colors cursor-pointer"
            >
