...
export function initializeSocket(token: string): Socket | null {
  ...
  if (!token) {
    console.warn("⚠️ Cannot initialize socket: No token provided");
    return null;
  }
  ...
  try {
    ...
    socket.on("connect", () => {
      ...
      console.log("✅ Socket.IO connected (Doctor Web)");
    });
    ...
    socket.on("disconnect", (reason) => {
      ...
      if (reason === "io server disconnect" || reason === "io client disconnect") {
        console.log("Socket.IO disconnected (Doctor Web):", reason);
        connectionErrorCount = 0;
        return;
      }
    });
    ...
    socket.on("connect_error", (error) => {
      ...
      if (process.env.NODE_ENV === "production") {
        console.warn(`Socket.IO connection error (Doctor Web) [${connectionErrorCount}/3]:`, error.message);
      }
    });

    socket.on("error", (error) => {
      if (process.env.NODE_ENV === "production") {
        console.error("Socket.IO error (Doctor Web):", error);
      }
    });
