...
export default function DashboardPage() {
  ...
  const operationsTiles = [
    { label: "IP Admissions", value: stats?.todayIP ?? 0, sub: "Today", color: "text-blue-700", onClick: () => router.push("/admin/patient-panel") },
    { label: "Emergencies", value: stats?.todayIPEmergency ?? 0, sub: "IP Emergency", color: (stats?.todayIPEmergency ?? 0) > 0 ? "text-red-600" : "text-slate-700", onClick: () => router.push("/admin/patient-panel") },
    { label: "OP Visits", value: stats?.todayOP ?? 0, sub: "Today", color: "text-emerald-700", onClick: () => router.push("/admin/reports") },
    { label: "Services Billed", value: stats?.todaySvc ?? 0, sub: "Today", color: "text-violet-700", onClick: () => router.push("/admin/reports") },
    { label: "Beds Occupied", value: stats?.bedOccupied ?? 0, sub: `of ${stats?.bedTotal ?? 0} total`, color: "text-orange-700", onClick: () => router.push("/admin/settings") },
    { label: "Bed Occupancy", value: `${stats?.bedOccupancyRate ?? 0}%`, sub: "Current rate", color: (stats?.bedOccupancyRate ?? 0) > 80 ? "text-red-600" : "text-emerald-700", onClick: () => router.push("/admin/settings") },
  ];
        ...
        <>
          ...
          >
            ...
            <div className="p-4 border-t border-slate-100 flex flex-wrap gap-3">
              {[
                { label: "IP Management", path: "/admin/patient-panel", color: "bg-blue-600" },
                { label: "Patient Summary", path: "/admin/patient-consolidated", color: "bg-indigo-600" },
                { label: "Rooms & Beds", path: "/admin/settings", color: "bg-orange-600" },
                { label: "Certificates", path: "/admin/reports", color: "bg-teal-600" },
                { label: "Hospital Reports", path: "/admin/reports", color: "bg-gray-700" },
              ].map((btn) => (
