...
export function initializeSocket(token: string): Socket | null {
  ...
  try {
    ...
    socket.on("connect", () => {
      ...
      console.log("✅ Socket.IO connected (Distributor)");
    });
    ...
    socket.on("disconnect", (reason) => {
      ...
      if (reason === "io server disconnect" || reason === "io client disconnect") {
        console.log("Socket.IO disconnected (Distributor):", reason);
        connectionErrorCount = 0;
        return;
      }
    });
    ...
    socket.on("connect_error", (error) => {
      ...
      if (process.env.NODE_ENV === "production") {
        console.warn(`Socket.IO connection error (Distributor) [${connectionErrorCount}/3]:`, error.message);
      }
    });
    ...
    if (manager) {
      manager.on("error", (error: any) => {
        if (process.env.NODE_ENV === "production") {
          console.error("Socket.IO transport error (Distributor):", error?.message || error);
        }
      });
      ...
      if (manager.engine) {
        manager.engine.on("error", (error: any) => {
          if (process.env.NODE_ENV === "production") {
            console.error("Socket.IO engine error (Distributor):", error?.message || error);
          }
        });
      }
    }
    ...
    socket.on("error", (error: any) => {
      if (process.env.NODE_ENV === "production") {
        console.error("Socket.IO error (Distributor):", error?.message || error);
      }
    });
